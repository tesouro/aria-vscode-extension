...
import type { PreviaPayload } from '../../core/types';
...
function buildParamsHtml(title: string, initialPayload: PreviaPayload): string {
...
export function openPreviewParamsWebview(
  title: string,
  initialPayload: PreviaPayload,
  onExecute: (payload: PreviaPayload) => Promise<void>
): void {
  ...
  panel.webview.onDidReceiveMessage(async (message: { command?: string; payload?: PreviaPayload }) => {
    if (message.command !== 'execute' || !message.payload) { return; }
