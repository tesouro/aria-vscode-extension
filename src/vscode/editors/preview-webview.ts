...
function buildPreviewHtml(title: string, initialPayload: PreviaPayload): string {
...
export function openPreviewWebview(
  ...
  panel.onDidDispose(() => {
    if (activePreviewPanel === panel) { activePreviewPanel = undefined; }
  }, undefined, context.subscriptions);

  panel.webview.onDidReceiveMessage(async (message: { command?: string; payload?: PreviaPayload }) => {
    if (message.command !== 'run' || !message.payload) { return; }
    try {
