...
function makeClient(): AnyClient {
  return new AriaApiClient({ baseUrl: 'https://test.example.com/', fetchProjectPath: '', ignoreSslErrors: true }) as
    unknown as AnyClient;
}

// ─── normalizeDataset (private, accessed via cast) ────────────────────────────

describe('AriaApiClient.normalizeDataset', () => {
  it('normalizes { registros: [...] } format', () => {
...
describe('AriaApiClient.mapProject', () => {
  it('maps numeric ID and string fields', () => {
    const client = makeClient();
    const raw = {
      ID_PROJETO: '42',
  ...
  it('uses lower-case aliases (id_projeto, nome_projeto, path_projeto)', () => {
    const client = makeClient();
    const raw = {
      id_projeto: 99,
  ...
  it('maps endpoints array from "endpoints" key', () => {
    const client = makeClient();
    const raw = {
      ID_PROJETO: 1,
      NO_PROJETO: 'P',
...
describe('AriaApiClient.mapEndpoint', () => {
  ...
  it('uses lower-case alias id_endpoint, nome_endpoint', () => {
    const client = makeClient();
  ...
  it('sets TX_CODIGO to undefined when not a string', () => {
    const client = makeClient();
...
describe('AriaApiClient.mapEndpointFormItem', () => {
  ...
  it('returns empty ITEM_NAME for missing input', () => {
    const client = makeClient();
...
describe('AriaApiClient.mapEndpointValidation', () => {
