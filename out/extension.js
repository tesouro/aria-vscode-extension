...
function getSafeUrl(value) {
    return (0, utils_1.toStringSafe)(value).trim();
}
async function openUrlInBrowser(urlValue, errorMessage) {
...
async function copyUrlToClipboard(urlValue, errorMessage, successMessage) {
    const url = getSafeUrl(urlValue);
    if (!url) {
        vscode.window.showWarningMessage(errorMessage);
        return;
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('aria.connect', async () => {
    ...
    }), vscode.commands.registerCommand('aria.openProjectDocumentation', async (node) => {
        if (!state.dataset || !node) {
            return;
        }
        const url = node.project.url ?? node.project.URL;
        if ((0, utils_1.toStringSafe)(url).trim()) {
    ...
    }), vscode.commands.registerCommand('aria.openProjectApex', async (node) => {
        if (!state.dataset || !node) {
            return;
        }
        const urlApex = node.project.url_apex ?? node.project.URL_APEX;
        if ((0, utils_1.toStringSafe)(urlApex).trim()) {
        ...
        if ((0, utils_1.toStringSafe)(endpointUrl).trim()) {
    ...
    }), vscode.commands.registerCommand('aria.openEndpointApex', async (node) => {
        if (!state.dataset || !node) {
            return;
        }
        const endpointApex = node.endpoint.url_apex ?? node.endpoint.URL_APEX;
        if ((0, utils_1.toStringSafe)(endpointApex).trim()) {
