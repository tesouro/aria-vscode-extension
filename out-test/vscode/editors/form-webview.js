...
exports.buildFormHtml = buildFormHtml;
exports.openFormWebview = openFormWebview;
...
function isValidateCodeSuccess(status) {
    const normalized = (0, utils_1.toStringSafe)(status).toLowerCase().trim();
    return normalized === 'sucesso' || normalized === 'ok' || normalized === 'success';
}
function getFieldSection(key) {
    if (['NO_PROJETO', 'NO_REST_CUSTOM', 'TX_PATH', 'DS_REST_CUSTOM_CURTA', 'ID_BANCO_EXTERNO', 'ID_BANCO_ESQUEMA', 'ID_TIPO_CODIGO', 'ID_METODO', 'NR_VERSAO'].includes(key)) {
        return 'basic';
    }
    if (['IN_TIPO_TRANSFORMACAO', 'IN_FORMATO_SAIDA', 'TX_SEPARADOR_CSV', 'ID_TIPO_HEADER', 'TX_MIME_TYPE', 'SN_PAGINADO', 'NR_PAGE_SIZE', 'SN_INCLUI_COUNT', 'SN_NULOS_EXPLICITOS', 'TX_CODIGO'].includes(key)) {
        return 'behavior';
    }
    if (['IN_MODO_SEGURANCA', 'TX_PERFIS', 'SN_EXIGE_OTP', 'ID_TIPO_OTP', 'TX_URL', 'SN_APENAS_INTERNO', 'TX_CODIGO_EMBED', 'TX_IPS', 'SN_PUBLICADO', 'SN_HABILITA_META_API', 'TX_SECRET_META_API', 'SN_MODO_COMPATIBILIDADE', 'SN_IGNORA_CONFIGS_DEPLOY'].includes(key)) {
        return 'security';
    }
    if (['SN_IDEMPOTENTE', 'SN_CACHE', 'NR_TEMPO_CACHE', 'IN_TEMPO_CACHE', 'IN_JANELA_TEMPO_CACHE', 'DT_EXP_CACHE'].includes(key)) {
        return 'cache';
    }
    if (/^ID_/.test(key)) {
        return 'metadata';
...
function buildFormHtml(title, data, excludeKeys, options) {
    ...
    const sectionOrder = endpointMeta
        ? (() => { const m = new Map(); for (const item of endpointMeta.values()) {
            if (!item.hidden && !m.has(item.region)) {
                m.set(item.region, item.regionSequence);
            }
        } return Array.from(m.entries()).sort((a, b) => a[1] - b[1]).map(([r]) => r); })()
    ...
    for (const [key, value] of visibleEntries) {
        ...
        const selectedProfileIds = (() => {
            ...
            const rawTokens = Array.isArray(value) ? value.map((i) => String(i ?? '').trim()).filter(Boolean) : (0, utils_1.parseListTokens)(strVal);
            if (!rawTokens.length) {
                return new Set();
            }
            const selected = new Set();
            for (const profile of options?.lovs?.PERFIL ?? []) {
                const pId = String(profile.ID_PERFIL);
                const pName = (0, utils_1.normalizeTextForLookup)(profile.NO_PERFIL);
                if (rawTokens.some((t) => t === pId || (0, utils_1.normalizeTextForLookup)(t) === pName)) {
                    selected.add(pId);
                }
            }
            return selected;
        ...
        if (isBoolean) {
            ...
            const rawTokens = Array.isArray(value) ? value.map((i) => String(i ?? '').trim()).filter(Boolean) : (0, utils_1.parseListTokens)(strVal);
            const selectedOtps = new Set();
            for (const otp of options.lovs.TIPO_OTP) {
                if (rawTokens.some((t) => t === String(otp.ID_TIPO_OTP) || (0, utils_1.normalizeTextForLookup)(t) === (0, utils_1.normalizeTextForLookup)(otp.NO_TIPO_OTP))) {
                    selectedOtps.add(String(otp.ID_TIPO_OTP));
                }
            }
...
function openFormWebview(context, title, data, excludeKeys, renderOptions, onSave, onValidate, onPreview) {
    ...
    panel.webview.onDidReceiveMessage(async (message) => {
        if (message.command === 'save') {
            try {
                ...
                void panel.webview.postMessage({ type: 'error', message: (0, utils_1.toErrorMessage)(error) });
            ...
            try {
                ...
                void panel.webview.postMessage({ type: 'validate-result', status: 'erro', mensagem: (0, utils_1.toErrorMessage)(error) });
            }
        }
            ...
            try {
                if (onValidate) {
                    ...
                    if (!isValidateCodeSuccess(validation.status)) {
                        return;
                    }
                }
                ...
                const result = await onPreview({
                    ...
                    valoresParametros: Array.isArray(d.valoresParametros) ? d.valoresParametros : [],
                });
